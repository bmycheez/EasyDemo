...
pipeline = [
    ...
    dict(type='GatePressKeyWrapper',
         ...
         transforms=[
             dict(type='TorchModelWrapper', model=dict(type='Unet4to4'), device='cuda',
                  alpha=0.4, load_from='weights/raw2raw-torch-2dnr/Unet4to4-8ch.pth',
                  input_key='noisy', output_key='denoised'),
             dict(type='TorchModelWrapper', model=dict(type='Unet4to4'), device='cuda',
                  alpha=0.8, load_from='weights/raw2raw-torch-2dnr/Unet4to4-8ch.pth',
                  input_key='noisy', output_key='denoised'),
             dict(type='TorchModelWrapper', model=dict(type='Unet4to4'), device='cuda',
                  alpha=1.2, load_from='weights/raw2raw-torch-2dnr/Unet4to4-8ch.pth',
                  input_key='noisy', output_key='denoised'),
             dict(type='TorchModelWrapper', model=dict(type='Unet4to4'), device='cuda',
                  alpha=1.6, load_from='weights/raw2raw-torch-2dnr/Unet4to4-8ch.pth',
                  input_key='noisy', output_key='denoised')]),
    # RAW2RGB
