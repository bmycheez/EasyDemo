...
pipeline = [
    ...
    dict(type='CollectFramesfor3DNR', frame_cnt=7),
    dict(type='GetRAWGreenMean', key='img_frame_list'),
    dict(type='ToTensor', keys=['img_frame_list'], device='cuda'),
    dict(type='KeyMapping', mapping={'img_frame_list': 'img'}),
    # ISP
    ...
    dict(type='GatePressKeyWrapper',
         ...
         init_state=4,
         use_auto_state=True,
         preprocessing=dict(type='NR3DSplit'),
         postprocessing=dict(type='NR3DMerge'),
         custom_state_name=["0.4", "0.8", "1.2", "1.6"],
         transforms=[
             dict(type='TorchModelWrapper', model=dict(type='Unet21to3'), device='cuda',
                  alpha=0.4, load_from='weights/rgb2rgb-torch-3dnr/Unet21to3-8ch.ckpt',
                  input_key='noisy', output_key='denoised'),
             dict(type='TorchModelWrapper', model=dict(type='Unet21to3'), device='cuda',
                  alpha=0.8, load_from='weights/rgb2rgb-torch-3dnr/Unet21to3-8ch.ckpt',
                  input_key='noisy', output_key='denoised'),
             dict(type='TorchModelWrapper', model=dict(type='Unet21to3'), device='cuda',
                  alpha=1.2, load_from='weights/rgb2rgb-torch-3dnr/Unet21to3-8ch.ckpt',
                  input_key='noisy', output_key='denoised'),
             dict(type='TorchModelWrapper', model=dict(type='Unet21to3'), device='cuda',
                  alpha=1.6, load_from='weights/rgb2rgb-torch-3dnr/Unet21to3-8ch.ckpt',
                  input_key='noisy', output_key='denoised')]),
    # Postprocessing
